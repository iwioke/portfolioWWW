...
test('poster slots open fullscreen, trap focus and return focus on Escape', async ({ page }) => {
  ...
  const trigger = page.getByRole('button', { name: 'Open Full composition fullscreen', exact: true })
  await trigger.click()
  ...
  await expect(page.locator('#media-viewer-title')).toHaveText('Vertical study')
...
test('lettering has its own fullscreen archive and honest dates', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Open Lettering composition fullscreen', exact: true }).click()
  ...
  await expect(page.locator('#media-viewer-title')).toHaveText('Letterform study')
...
test('reduced motion keeps content and interactions usable', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible()
  await page.goto('/work/posters')
  await page.getByRole('button', { name: 'Open Full composition fullscreen', exact: true }).click()
